...
export class JeuRouter {
  ...
  public redemarrerJeu(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const joueursObj = JSON.parse(this._controleurJeu.joueurs);
  ...
  public joueurs(req: Request, res: Response, next: NextFunction) {
    try {
      const joueursObj = JSON.parse(this._controleurJeu.joueurs);
      res.status(200).send(joueursObj);
    } catch (error) {
      this._errorCode500(error, req, res);
    }
  }
  ...
  init() {
    ...
    this._router.get(('/joueurs'), this.joueurs.bind(this)); // pour .bind voir https://stackoverflow.com/a/15605064/1168342
  }
